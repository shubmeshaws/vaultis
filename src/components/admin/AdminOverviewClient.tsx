                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h1 className="text-4xl font-bold tracking-tighter text-foreground leading-tight">
                            Admin <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-purple-500 to-blue-500">Overview</span>
                        </h1>
                        <p className="text-sm text-muted-foreground font-normal mt-2 max-w-xl">System Administration & Monitoring</p>
                    </div>
                </div>

                <div className="flex items-center gap-3 mt-8 md:self-end">
                    <Link href="/settings">
                        <button className="h-10 px-6 bg-foreground/5 hover:bg-foreground/10 text-foreground border border-foreground/5 rounded-2xl text-[10px] font-bold uppercase tracking-widest flex items-center gap-3 transition-all">
                            <Settings className="w-4 h-4" />
                            System Config
                        </button>
                    </Link>
                    <Link href="/admin/analyzer">
                        <button className="h-10 px-6 bg-primary hover:bg-primary/90 text-primary-foreground rounded-2xl text-[10px] font-bold uppercase tracking-widest flex items-center gap-3 transition-all shadow-xl shadow-primary/20 active:scale-95">
                    ...
                    <Card className="border-foreground/10 bg-white dark:bg-foreground/[0.02] backdrop-blur-3xl rounded-[2.5rem] overflow-hidden shadow-2xl shadow-black/5">
                        <CardHeader className="flex flex-row items-center justify-between p-8 border-b border-foreground/5 bg-foreground/[0.01]">
                            <div className="space-y-1">
                                <CardTitle className="text-2xl font-bold tracking-tight">Recent Registrations</CardTitle>
                                <CardDescription className="text-xs font-normal uppercase tracking-[0.1em] text-muted-foreground">Identity & Permission Management</CardDescription>
                            </div>
                                ...
                                {recentUsers.map(u => (
                                    <div key={u.id} className="flex items-center justify-between p-5 rounded-3xl bg-foreground/[0.02] border border-foreground/5 hover:border-primary/20 hover:bg-primary/[0.02] transition-all group cursor-pointer">
                                        <div className="flex items-center gap-5">
                                            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-foreground/5 flex items-center justify-center text-xs font-bold text-muted-foreground uppercase group-hover:scale-110 transition-transform">
                                                {u.email?.substring(0, 2)}
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{u.name || 'Unnamed User'}</p>
                                                <p className="text-[10px] font-mono text-muted-foreground tracking-tighter">{u.email}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <span className={cn(
                                                "px-3 py-1.5 rounded-xl text-[9px] font-bold uppercase tracking-widest border transition-all",
                        ...
                        <Card className="border-purple-500/20 bg-gradient-to-br from-purple-500/10 via-indigo-500/10 to-purple-500/10 backdrop-blur-3xl rounded-[2rem] overflow-hidden shadow-xl shadow-purple-500/5 group">
                            ...
                            <CardHeader className="py-6 px-8 border-b border-purple-500/10">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-[9px] font-bold text-purple-500 uppercase tracking-[0.2em]">Privileged Access</span>
                                </div>
                                <CardTitle className="text-4xl font-bold tracking-tighter text-foreground">{stats.adminCount}</CardTitle>
                            ...
                            <CardContent className="p-8">
                                ...
                                <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-purple-500 opacity-60">
                    ...
                    <motion.div variants={itemVariants}>
                        <Card className="border-foreground/10 bg-white dark:bg-foreground/[0.01] backdrop-blur-3xl rounded-[2rem] overflow-hidden shadow-sm">
                            <CardHeader className="py-6 px-8 border-b border-foreground/5">
                                <CardTitle className="text-sm font-bold uppercase tracking-widest">Global Policies</CardTitle>
                            </CardHeader>
                            <CardContent className="p-4 space-y-1">
                                {[
                                    ...
                                    <Link key={i} href={link.href} className="flex items-center justify-between p-4 rounded-2xl hover:bg-foreground/5 transition-colors group">
                                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest group-hover:text-foreground transition-colors">{link.label}</span>
