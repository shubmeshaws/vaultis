            ...
            >
                <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="flex flex-col items-start min-w-0">
                        <span className="text-xs font-bold text-white/90 truncate">{selectedDb.name}</span>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute top-[calc(100%+8px)] left-0 w-[calc(100%+20px)] -ml-[10px] bg-[#0A0A0E]/90 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl p-2 z-50 overflow-hidden ring-1 ring-black/50"
                    >
                        {/* Header */}
                        <div className="px-2 py-1.5 mb-1 flex items-center justify-between text-[10px] font-bold text-white/20 uppercase tracking-widest border-b border-white/5">
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className={cn(
                                            ...
                                            isSelected ? "bg-indigo-500 text-white border-indigo-400" : "bg-white/5 text-white/40 border-white/5 group-hover:border-white/10",
                                            isLocked && "bg-black/40 border-white/5"
                                        )}>
                                        ...
                                        <div className="flex-1 min-w-0 z-10">
                                            <div className="flex items-center justify-between mb-0.5">
                                                ...
                                                {isLocked && <span className="text-[9px] font-black uppercase text-red-500/70 border border-red-500/20 px-1 py-0.5 rounded">Locked</span>}
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", getStatusColor(db.status))} />
                                                <span className="text-[10px] text-white/30 font-medium uppercase truncate">{db.permission.replace('_', ' ')}</span>
                                            </div>
                                        </div>
                        ...
                        <div className="mt-2 pt-2 border-t border-white/5 px-1">
                            <button className="w-full py-2 flex items-center justify-center gap-2 rounded-lg border border-dashed border-white/10 text-[10px] font-bold text-white/30 hover:text-white hover:border-white/30 hover:bg-white/5 transition-all uppercase tracking-wide">
                                + Add Connection
                            </button>
                        </div>
                    </motion.div>
